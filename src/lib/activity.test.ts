...
const field = (id: string, at: string): FieldChangeEntry => ({
  id,
  field: "in_program",
  fromValue: "false",
  toValue: "true",
...
const shape = (items: ReturnType<typeof buildActivity<TestComment>>) =>
  items.map(
    (i) => `${i.kind}:${i.kind === "comment" ? i.node.comment.id : i.entry.id}`,
  );

describe("buildActivity", () => {
  it("returns nothing when all inputs are empty", () => {
  ...
  it("keeps comments oldest-first when history is empty", () => {
    const items = buildActivity(
      [],
      [],
      [root("c1", "2026-08-02"), root("c2", "2026-08-05")],
    );
    expect(shape(items)).toEqual(["comment:c1", "comment:c2"]);
  });

  it("interleaves all three streams by time", () => {
    const items = buildActivity(
  ...
  it("keeps a reply under its root, not at its own timestamp", () => {
    ...
    expect(
      items[1].kind === "comment" && items[1].node.children[0].comment.id,
    ).toBe("c1a");
  });

  it("breaks a timestamp tie status-first, then field, then comment, then id", () => {
    // The field-after-status rule is what makes a promotion's auto-admit
    // read as its consequence rather than its cause.
